...
尚未接入，真实支付、账号计时和桌面壳仍未实现。HTML 的定位是试用系统：有购买意愿请联系项目方，正式注册用户可获得后续版本更新与升级服务。
当前页面不接入支付或账号注册。商业模式与数据闭环见 [BUSINESS_MODEL.md](BUSINESS_MODEL.md)。
...
当前 HTML 是本地试用系统，不是在线支付页。用户可以先本地体验照片预览、三类筛选、导出和本机 AI 识别；有购买意愿请通过项目 [GitHub 仓库](https://github.com/chuanxituzhu-lab/sayelf-birdpick) 联系项目方，并以正式公布的价格、服务范围和条款为准。正式版注册用户可获得版本更新与升级服务；当前独立 HTML 不创建账号、不收集手机号或邮箱，也不会自动发送通知，注册入口和联系方式需要项目方后续正式公布。

### 工作流原则与本次运行状态

页面按“打开 → 导入 → 执行 → 结果”组织，并在页面顶部显示本次工作流的四个阶段：导入、本地 AI（可选）、人工复核、导出结果。状态卡会区分等待导入、处理中、待人工复核、部分失败、可以导出和已生成结果；“本次运行记录”只记录本机会话中的阶段变化、数量和复核状态，不记录文件名，不上传到云端。导出的本地 JSON 会带运行编号、结果版本和复核状态：全部照片完成三类人工确认才标记为 `PASS`，仍有“待定”时标记为 `NEEDS_REVIEW`，不把未复核结果冒充训练真值。
